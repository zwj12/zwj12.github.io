...
### RST跟FIN的区别：
正常关闭连接的时候发的包是FIN，但是如果是异常关闭连接，则发送RST包，两者的区别在于：
...
2. 请求超时，比如socket，设置RST超时的时间为100ms，但是抓包发现，从发到收花的时间超过了100ms，那么主机就会认为这个包超时了，就会发送RST包拒绝此连接
3. 劫持，客户端向服务端发起请求，但是这时候有冒充服务端来回包给我的客户端，但是因为ACK的值没能对上所以发送了RST包给狠狠地拒绝了
4. 提前关闭，‘TCP是一种可靠的连接’。 而这可靠有这样一种含义，那就是操作系统接收到的来自TCP连接中的每一个字节，我都会让应用程序接收到。如果应用程序不接收怎么办？你猜对了，RST。
